...
import LoginTransitionLoader from "@/components/LoginTransitionLoader"; // ← IMPORT
...
export default function Login() {
  ...
  const [showLoader, setShowLoader] = useState(false); // ← État pour le loader
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.token) {
        ...
        // Petite pause pour que le loader soit visible
        setTimeout(() => {
          router.replace("/admin");
        }, 500);
      } else {
  ...
  return (
    <>
      <LoginTransitionLoader /> {/* ← TOUJOURS PRÉSENT */}
